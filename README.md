...
- [Полный перенос из GitHub](#полный-перенос-из-github)
...
### На новом ПК: клонирование → подготовка → запуск

Нужны **Git, Node.js 22.14+ с npm и установленный PostgreSQL**
(проверено на PostgreSQL 17). Docker, Figma и исходный компьютер не нужны.
При установке PostgreSQL оставьте компонент **Command Line Tools** и
запомните пароль администратора `postgres`. Первый запуск требует интернета
для установки npm-зависимостей.

```powershell
git clone https://github.com/bordermull/creator-platform.git
cd creator-platform
npm run setup
npm start
```

**Готовый сайт:** [http://127.0.0.1:4173](http://127.0.0.1:4173).
Вход администратора: `admin@creatur.local` / `password123`.
Обычный автор: `viktor@creatur.local` / `password123`.
Остановка обоих серверов — `Ctrl+C` в терминале запуска.

Папка может быть любой — `C:\repositories` не обязателен, пробелы в пути
поддерживаются. В Windows можно также открыть `setup.cmd`, затем `start.cmd`.
Сам Node.js и PostgreSQL эти файлы не устанавливают.

### Что подготовка делает сама

- Устанавливает backend-зависимости **по package-lock** через `npm ci`.
- Создаёт `backend/.env`, если его ещё нет, и генерирует отдельный случайный
  секрет сессии. Уже существующий `.env` не перезаписывает.
- Проверяет соединение с PostgreSQL. Для стандартной локальной установки,
  если БД ещё нет, предлагает создать `creatur` и роль `creatur` через `psql`.
  Пароль администратора запрашивает сам PostgreSQL; он не сохраняется в Git.
- Применяет схему без `--accept-data-loss` и без удаления существующей БД.
- Проверяет комплект по SHA-256, копирует демонстрационные загрузки и
  восстанавливает пользователей, проекты, категории и связи.
- Собирает TypeScript. Команда `npm start` затем запускает frontend и backend
  вместе и проверяет HTTP-готовность, включая доступ API к базе.

Повторный `npm run setup` добавляет недостающие демо-записи, но **не сбрасывает**
пользовательские пароли, профили и правки проектов. Перед ним остановите серверы
этой копии, чтобы Windows не блокировала замену используемых библиотек.
Для обычного следующего запуска достаточно `npm start`.

### Если PostgreSQL уже настроен иначе

Укажите свою строку `DATABASE_URL` в `backend/.env`, затем повторите
`npm run setup`. Автоматическое создание БД рассчитано только на стандартную
локальную пару `creatur/creatur`; чужие базы и пароли скрипт не меняет.
Если `psql` не найден, добавьте его в PATH либо задайте `PSQL_PATH` полным
путём к `psql.exe`. Если порты 3000/4173 заняты, `npm start` сообщит об этом
и не станет завершать посторонние процессы.

### Вообще без PostgreSQL
...
Это та же полная демонстрация с теми же данными; отличается только БД.
Основной рекомендуемый для сдачи режим остаётся PostgreSQL.

## Полный перенос из GitHub

После подтверждения заказчика о демонстрационном характере всех данных
в репозиторий добавлен **полный переносимый комплект**:
[backend/demo](backend/demo/README.md). Больше не нужно запрашивать дамп
у автора или копировать файлы с исходного ПК для воспроизведения этой версии.

| В чистой установке | Количество |
| --- | ---: |
| Пользователи | 20, включая одного администратора |
| Проекты | 19: 12 опубликованных, 6 на модерации, 1 отклонённый |
| Файлы проектов | 19: 14 ресурсов frontend и 5 защищённых загрузок |
| Категории | 34 |
| Связи проектов с категориями | 78 |

Все **новые аккаунты из комплекта** получают пароль `password123`.
Имена, email, описания, иллюстрации, статусы и принадлежность работ сохранены.
Пароли старой установки и токены сброса намеренно не копируются: для показа
они не нужны. При необходимости список email доступен в админке или снимке.

Сохранены и старые технические smoke-примеры, которые не видны гостю.
Они могут содержать пустые проекты и минимальные тестовые файлы; это отдельно
объяснено в [описании комплекта](backend/demo/README.md#старые-технические-примеры).
Это не 19 отобранных публичных портфолио: готовый каталог содержит 12 работ.

Живая PostgreSQL-база, рабочий `uploads`, `.env` и локальные письма по-прежнему
не коммитятся. В Git лежит **контролируемый снимок** и копии демо-файлов,
которые при подготовке превращаются в обычные записи и защищённые загрузки.
Если позже на исходном ПК добавятся новые работы, они не появятся в Git сами:
комплект нужно сознательно обновить и проверить перед публикацией.

Для проверки **чистой** установки:
...
Первая команда сверяет количества записей, связи, наличие и контрольные суммы
всех файлов. Вторая проходит сценарии приложения, создавая и удаляя только
собственные тестовые данные. После пользовательских изменений `demo:verify`
может закономерно сообщить отличие от исходного снимка — это не сброс базы.

3D-просмотрщик теперь включён в Git вместе с лицензией, а не загружается
с CDN. Обычный самодостаточный GLB работает локально; специальные сжатые
модели и AR могут требовать внешних декодеров. Установка npm-зависимостей
на новом ПК всё равно требует интернет, если они не закэшированы.

## Запуск с PostgreSQL

PostgreSQL — основной режим разработки проекта. Ниже — ручной вариант для
понимания отдельных шагов; новичку достаточно `npm run setup` из раздела выше.
...
SQLite оставлен как отдельный режим “быстрый запуск без PostgreSQL”.
Из корня проекта используйте `npm run setup:sqlite`, затем `npm start`.
Для ручного запуска сначала создайте `backend/.env` из `.env.example`.
Оба варианта seed импортируют полный снимок из `backend/demo`.
...
.
├── backend/
│   ├── demo/                     Полный JSON-снимок, файлы, контрольные суммы
│   ├── prisma/
│   │   ├── schema.prisma          PostgreSQL Prisma schema
│   │   ├── schema.sqlite.prisma   SQLite fallback schema
│   │   ├── init-sqlite.mjs        Создание SQLite dev.db
│   │   ├── seed-demo.mjs          Основное заполнение из переносимого снимка
│   │   ├── export-demo.mjs        Явный экспорт подтверждённых демо-данных
│   │   ├── verify-demo.mjs        Проверка полного восстановления
│   │   └── seed.ts               Старый программный seed, сохранён для истории
│   ├── src/
...
│   ├── vendor/model-viewer/       Локальный 3D-viewer и его лицензия
...
Используется `<model-viewer>` из `@google/model-viewer@3.5.0`.
Его официальный bundle включён в `frontend/vendor/model-viewer` вместе
с лицензией Apache-2.0 и подключён локально в `frontend/index.html`.
...
Перенос проверен отдельным сценарием: чистые копии **только файлов из Git**
в каталогах с пробелами, новая PostgreSQL-база и отдельная SQLite-база.
Для обоих режимов успешно выполнены подготовка с нуля, проверка всех записей
и контрольных сумм файлов, повторный seed без дубликатов, API-тесты и единый
запуск обоих серверов с последующей остановкой. Рабочая база не очищалась.

Разработчик может повторить это командой `npm run test:transfer` из корня.
Она проверяет текущий **Git index**, поэтому сначала подготовьте проверяемые
файлы через `git add`. Нужны свободные порты 3000/4173, интернет для npm
и право `CREATEDB` у локального PostgreSQL-пользователя.
Созданная тестом БД с уникальным именем удаляется автоматически; чистые копии
в системном Temp остаются для осмотра. Для обычного пользователя этот тяжёлый
тест не нужен — достаточно подготовки и запуска из раздела выше.
...
Исключение по назначению: **`backend/demo` коммитится сознательно**.
Это подтверждённый заказчиком демонстрационный комплект без исходных
passwordHash, токенов и настроек подключения, а не автоматическая выгрузка
любых будущих пользовательских данных.
