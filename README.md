...
Страница проекта также подключена к backend: она открывается мгновенно из уже известных данных каталога/fallback, а затем обновляется через `GET /api/projects/:id`. Это сохраняет быстрый переход между страницами и постепенно переводит detail view на backend truth.
...
Загруженные файлы отдаются backend через `/uploads/...`. Поэтому frontend различает два типа изображений: старые Figma/export ассеты остаются локальными `/assets/...`, а новые upload-файлы открываются с `http://127.0.0.1:3000/uploads/...`.
...
- `GET /api/projects/:id` — detail DTO конкретного проекта;
...
- `GET /uploads/:file` — локальная раздача загруженных файлов в dev-режиме;
...
flowchart TD
  ...
  detail["Open #project/:id<br/>detail DTO"]
...
- `/uploads/<file>` вернул `200` для загруженного файла;
...
3. Доработать страницу проекта под реальные `files[]`: галерея, видео, документы и 3D/model-файлы.
