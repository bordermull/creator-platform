...
- [Доработка по Figma и перенос данных](#доработка-по-figma-и-перенос-данных)
...
- лайки проектов;
- редактирование имени, описания, специализации, контакта, города, аватара и обложки;
- публичные страницы авторов с реальными данными;
- подписки на авторов и отдельная страница подписок;
- закладки опубликованных проектов;
- комментарии с удалением автором комментария или администратором;
- удаление проекта владельцем/администратором через архивирование;
- формы запроса и подтверждения сброса пароля;
...
Из нового макета добавлены четыре обычных автора: `viktor@creatur.local`,
`mel@creatur.local`, `jinx@creatur.local`, `jayce@creatur.local`.
Демонстрационный пароль каждого — `password123`. Их имена, описания, даты
регистрации, иллюстрации и проекты соответствуют блоку «Люди в БД» макета.
Открытая регистрация всегда создаёт `USER`; администратора создаёт seed.

## Доработка по Figma и перенос данных

Актуальный макет: [CREATUR в Figma](https://www.figma.com/design/XGYHDH6lleb0UyfCyi1nBI/Untitled--Copy-?node-id=0-1).
Полный журнал решений, резервного копирования и проверок: [доработка от 08.09.2026](docs/upgrade-2026-09-08.md).

Ниже — снимок работающего приложения с данными PostgreSQL, а не изображение макета.

![Итоговая страница автора CREATUR](docs/profile-final.png)

Ещё примеры: [страница проекта](docs/project-final.png), [подписки](docs/following-final.png).

PostgreSQL — основной источник пользователей, проектов, категорий, связей,
реакций и настроек профиля. При отключении API каталог показывает ошибку
подключения, а не подменяет базу демонстрационным массивом JavaScript.
Файлы пользовательских проектов физически хранятся в `backend/uploads`,
а их метаданные и права доступа — в PostgreSQL. Аватары и обложки, загруженные
через редактор, сохраняются в БД; ограничение — по 2 МБ на изображение.

### Перенос старой SQLite-базы

Сначала сохраните копию проекта, `uploads` и выгрузку целевой PostgreSQL-базы.
Не запускайте перенос с пустой/непроверенной резервной копией.
Выполняйте команды из `backend`, используя Node.js 22.14 или новее:

```powershell
npm run prisma:push:postgres
npm run import:sqlite
npm run import:sqlite -- --apply
```

Первая команда импорта — пробная: проверяет все операции в транзакции и
откатывает их. `--apply` сохраняет результат. Для другого файла добавьте
`--source=C:/path/to/dev.db`. SQLite открывается только для чтения.
Пользователи сопоставляются по email, категории — по группе и slug.
Существующие профили/пароли/статусы PostgreSQL не перезаписываются.
Недостающие записи добавляются с сохранением связей. Новые импортированные
аккаунты получают `USER`, чтобы не добавлять старых тестовых администраторов.
Повторный импорт не создаёт дубликаты. Различающиеся старые значения остаются
в исходной SQLite-базе и резервной копии.

Seed тоже сохраняет пользовательские изменения: существующие проекты и их
файлы не заменяются демонстрационными. Изображения Figma сохранены в репозитории,
поэтому временные ссылки сервиса не нужны для запуска на другом ПК.
...
- понятное сообщение об ошибке при недоступном backend.
...
Если backend выключен или недоступен, каталог сообщает об ошибке соединения. Работа с данными требует запущенного API; SQLite остаётся отдельным режимом запуска API, а не подменой данных во frontend.
...
| `DELETE` | `/api/projects/:id` | Архивирование владельцем или администратором |
| `POST` | `/api/projects/:id/view` | Учёт открытия опубликованного проекта |
| `GET` | `/api/projects/:id/activity` | Комментарии и состояние лайка/закладки |
| `POST` | `/api/projects/:id/comments` | Добавление комментария |
| `DELETE` | `/api/projects/:id/comments/:commentId` | Удаление своего комментария или модерация |
| `POST / DELETE` | `/api/projects/:id/bookmark` | Добавление/удаление закладки |
| `GET` | `/api/users/:id` | Публичный профиль и опубликованные работы |
| `PATCH` | `/api/users/:id` | Изменение профиля владельцем/администратором |
| `POST / DELETE` | `/api/users/:id/follow` | Подписка/отписка |
| `GET` | `/api/users/me/following` | Подписки текущего пользователя |
| `GET` | `/api/users/me/bookmarks` | Сохранённые опубликованные работы |
...
erDiagram
  ...
  Project ||--o{ Bookmark : saved_in
  Project ||--o{ Comment : discussed_in
...
flowchart LR
  ...
  archived["ARCHIVED<br/>удалён из интерфейса"]

  draft -->|"submit"| pending
  pending -->|"admin publish"| published
  pending -->|"admin reject + reason"| rejected
  rejected -->|"edit + resubmit"| pending
  draft -->|"owner/admin delete"| archived
  pending -->|"owner/admin delete"| archived
  published -->|"owner/admin delete"| archived
  rejected -->|"owner/admin delete"| archived
```
...
- `ARCHIVED` скрывает работу и preview даже от владельца; исходные файлы
  сохраняются для ручного восстановления. Автоматическая очистка не запускается.
...
Если `SMTP_URL` пустой, backend использует JSON transport: письмо формируется, но реально не отправляется. Оно сохраняется в `backend/.local-mail/*.json`; ссылку сброса можно открыть из поля `text`. Каталог не отдаётся через HTTP и исключён из Git. Для настоящей доставки задайте `SMTP_URL` в локальном `.env`.

## Проверки

Полная автоматическая проверка API (из каталога `backend`):

```powershell
npm test
```

Тест использует текущую БД из `.env`, создаёт временных пользователей и проверяет
регистрацию, права, профиль, изображения профиля, приватность файлов, полный цикл
модерации, подписки без дубликатов, лайки/закладки/комментарии, архивирование и
сброс пароля. В `finally` удаляются только тестовые записи и файлы.
...
- `*.db`;
- `*.log`.
- `.local-mail/` — локальные письма и ссылки сброса;
- `*.dump` — выгрузки базы с приватными данными.
